...
/**
	...
	* @param controlByte The control byte used to initialize the chip can be generated using the PCF_CONTROL_BYTE macro.
...
{
	IIC_TRANSMIT(PCF8951_ADR_W, &controlByte, 1);
...
{
	IIC_TRANSMIT(PCF8951_ADR_W, pdata, size);
	return IIC_ERROR_FLAG;
}
...
{
	IIC_RE(PCF8951_ADR_R, pdata, size);
	return IIC_ERROR_FLAG;
}
